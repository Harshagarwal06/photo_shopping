...
def test_comparison_routes_are_exposed_without_checkout_routes():
    ...
    assert "/api/contracts/preview" in paths
    assert "/api/contracts/{contract_id}/confirm" in paths
    ...
    assert "/api/comparisons/proposals/{proposal_id}/receipt" in paths
    assert "/api/comparisons/{operation_id}/receipt" in paths
    assert not any(
